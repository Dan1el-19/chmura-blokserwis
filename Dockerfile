# Stage 1: Build
FROM node:25-alpine AS builder

RUN npm install -g pnpm@11.0.8

WORKDIR /app

COPY package.json pnpm-lock.yaml pnpm-workspace.yaml ./
RUN pnpm install --frozen-lockfile

COPY . .

ARG PUBLIC_APPWRITE_PROJECT_ID
ARG PUBLIC_APPWRITE_PROJECT_NAME
ARG PUBLIC_APPWRITE_ENDPOINT
ARG APPWRITE_API_KEY
ARG RELEASES_APPWRITE_ENDPOINT
ARG RELEASES_APPWRITE_PROJECT_ID
ARG RELEASES_APPWRITE_API_KEY
ARG R2_ENDPOINT
ARG R2_ACCESS_KEY_ID
ARG R2_SECRET_ACCESS_KEY
ARG R2_BUCKET_NAME
ARG UPSTASH_REDIS_REST_URL
ARG UPSTASH_REDIS_REST_TOKEN

ENV PUBLIC_APPWRITE_PROJECT_ID=$PUBLIC_APPWRITE_PROJECT_ID
ENV PUBLIC_APPWRITE_PROJECT_NAME=$PUBLIC_APPWRITE_PROJECT_NAME
ENV PUBLIC_APPWRITE_ENDPOINT=$PUBLIC_APPWRITE_ENDPOINT
ENV APPWRITE_API_KEY=$APPWRITE_API_KEY
ENV RELEASES_APPWRITE_ENDPOINT=$RELEASES_APPWRITE_ENDPOINT
ENV RELEASES_APPWRITE_PROJECT_ID=$RELEASES_APPWRITE_PROJECT_ID
ENV RELEASES_APPWRITE_API_KEY=$RELEASES_APPWRITE_API_KEY
ENV R2_ENDPOINT=$R2_ENDPOINT
ENV R2_ACCESS_KEY_ID=$R2_ACCESS_KEY_ID
ENV R2_SECRET_ACCESS_KEY=$R2_SECRET_ACCESS_KEY
ENV R2_BUCKET_NAME=$R2_BUCKET_NAME
ENV UPSTASH_REDIS_REST_URL=$UPSTASH_REDIS_REST_URL
ENV UPSTASH_REDIS_REST_TOKEN=$UPSTASH_REDIS_REST_TOKEN

RUN pnpm build

RUN pnpm prune --prod


# Stage 2: Production
FROM node:25-alpine AS production

WORKDIR /app

COPY --from=builder /app/build build/
COPY --from=builder /app/node_modules node_modules/
COPY --from=builder /app/package.json .

ENV NODE_ENV=production
ENV HOST=0.0.0.0
ENV PORT=3000

EXPOSE 3000

CMD ["node", "build"]
