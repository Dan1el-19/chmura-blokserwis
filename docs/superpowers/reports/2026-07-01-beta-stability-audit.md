...
Beta jest wdrożona na środowisko beta i gotowa do promocji na stable po uwzględnieniu aktualnego `origin/main`.
...
- `origin/main` ma nowy commit względem brancha beta; promocja stable wymaga rebase/merge z `origin/main`, ponownych bramek i dopiero wtedy deployu stable.

## Rekomendowana kolejność promocji

1. Zrebase'ować albo zmergować branch beta z aktualnym `origin/main`.
2. Powtórzyć bramki lokalne i CI po integracji z `origin/main`.
3. Wypchnąć promocję do `main`.
4. Poczekać na `Deploy stable`.
5. Wykonać smoke stable: `/`, `/login`, public preview, admin storage, upload fail/complete, releases.
