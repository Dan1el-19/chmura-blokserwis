...
export default function NewFolderModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^[\w\-. ]{1,64}$/.test(name)) {
      toast.error("Nieprawidłowa nazwa");
      return;
    }
    setLoading(true);
    try {
      ...
      const res = await fetch("/api/files/folder", {
        ...
        body: JSON.stringify({ folder: currentSpace, name }),
      });
      if (!res.ok) {
        const { error } = await res
          .json()
          .catch(() => ({ error: "Błąd tworzenia folderu" }));
  ...
  return (
    <div
      className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-[10000] p-0 sm:p-4"
      role="dialog"
      aria-modal="true"
          ...
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            <div>
              ...
              <p className="mt-1 text-[10px] sm:text-xs text-gray-500">
                Dozwolone: litery, cyfry, spacje, _ - . (max 64)
              </p>
            </div>
