  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/files/stats?fileKey=${encodeURIComponent(fileKey)}`,
        {
          headers: {
            ...
            <div className="space-y-4 sm:space-y-6">
                      ...
                      <table className="w-full min-w-[500px]">
                        <thead>
                          <tr className="border-b border-gray-200">
                            <th className="text-left py-2 px-2 sm:px-3 text-xs sm:text-sm font-medium text-gray-900">
                              Nazwa linku
                            </th>
                            <th className="text-center py-2 px-2 sm:px-3 text-xs sm:text-sm font-medium text-gray-900">
                              Kliknięcia
                            </th>
                            <th className="text-center py-2 px-2 sm:px-3 text-xs sm:text-sm font-medium text-gray-900">
                              Unikalni
                            </th>
                            <th className="text-center py-2 px-2 sm:px-3 text-xs sm:text-sm font-medium text-gray-900 hidden sm:table-cell">
                              Ostatni dostęp
                            </th>
                            <th className="text-center py-2 px-2 sm:px-3 text-xs sm:text-sm font-medium text-gray-900 hidden sm:table-cell">
                              Utworzony
                        ...
                        <tbody>
                          {stats.linkStats.map((linkStat) => (
                            ...
                            >
                              <td className="py-2 sm:py-3 px-2 sm:px-3 text-xs sm:text-sm text-gray-900 truncate max-w-[150px]">
                                {linkStat.linkName}
                              </td>
                              <td className="py-2 sm:py-3 px-2 sm:px-3 text-xs sm:text-sm text-center text-gray-900">
                                {linkStat.totalClicks}
                              </td>
                              <td className="py-2 sm:py-3 px-2 sm:px-3 text-xs sm:text-sm text-center text-gray-900">
                                {linkStat.uniqueVisitors}
                              </td>
                              <td className="py-2 sm:py-3 px-2 sm:px-3 text-xs sm:text-sm text-center text-gray-600 hidden sm:table-cell">
                                {linkStat.lastAccessed
                                  ? formatDate(linkStat.lastAccessed)
                                  : "Brak"}
                              </td>
                              <td className="py-2 sm:py-3 px-2 sm:px-3 text-xs sm:text-sm text-center text-gray-600 hidden sm:table-cell">
                                {linkStat.createdAt
                                  ? formatDate(linkStat.createdAt)
                                  : "Brak"}
                              </td>
                            </tr>
                          ))}
                          ...
                          <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-[10px] sm:text-xs text-gray-600">
                                {formatDate(activity.accessedAt)} •{" "}
              ...
              {stats.totalClicks === 0 && (
                <div className="text-center p-6 sm:p-8">
                  ...
                  <p className="text-gray-600 text-sm">
                    Brak aktywności dla tego pliku
                  </p>
                  <p className="text-xs sm:text-sm text-gray-500 mt-2">
                    Statystyki pojawią się po pierwszym użyciu linku
                  </p>
                </div>
              )}
            </div>
