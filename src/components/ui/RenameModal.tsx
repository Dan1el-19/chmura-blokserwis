...
export default function RenameModal({ isOpen, onClose, fileKey, currentName, onRenamed }: RenameModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start sm:items-center justify-center z-[12000] p-4 pt-[20vh] sm:pt-4 overflow-y-auto" role="dialog" aria-modal="true" aria-label="Zmień nazwę pliku">
      <Card className="w-full max-w-sm rounded-xl shadow-xl">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5">Nowa nazwa</label>
              <input
                ref={inputRef}
                value={value}
            ...
            <div className="flex justify-end gap-2 pt-2">
              <Button type="button" variant="outline" onClick={onClose} className="text-sm py-2.5">Anuluj</Button>
              <Button type="submit" variant="primary" disabled={loading || !value.trim() || value.trim()===currentName} className="text-sm py-2.5">
