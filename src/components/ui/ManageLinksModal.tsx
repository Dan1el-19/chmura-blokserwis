    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <Card className="w-full sm:max-w-4xl rounded-t-2xl sm:rounded-xl max-h-[90vh] overflow-hidden">
        <CardHeader className="flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur-sm z-10">
          <h3 className="text-base sm:text-lg font-semibold text-gray-900 truncate pr-2">Zarządzanie linkami</h3>
          <Button
            variant="ghost"
            size="sm"
            onClick={onClose}
        ...
        <CardContent className="overflow-y-auto max-h-[calc(90vh-120px)] pb-safe">
          <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <p className="text-xs sm:text-sm text-gray-600 truncate">Plik: <span className="font-medium text-gray-900 break-all">{fileName}</span></p>
            <Button
              variant="outline"
              size="sm"
          ...
          {loading ? (
            ...
            <div className="text-center p-4">
              ...
              <Button variant="outline" onClick={fetchLinks} className="mt-2 text-xs sm:text-sm">
                Spróbuj ponownie
              </Button>
            </div>
          ) : links.length === 0 ? (
            <div className="text-center p-6 sm:p-8">
              <p className="text-gray-600 text-sm">Brak aktywnych linków dla tego pliku</p>
            </div>
          ) : (
            <div className="space-y-3 sm:space-y-4">
              {links.map((link) => (
                <div key={link.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
                  {editingLink === link.id ? (
                    // Tryb edycji
                    <div className="space-y-3 sm:space-y-4">
                      <div>
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <label htmlFor={`relative-edit-${link.id}`} className="flex items-center gap-2 text-xs sm:text-sm font-medium text-gray-900">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="text-gray-900"
                          />
                          <label htmlFor={`custom-edit-${link.id}`} className="flex items-center gap-2 text-xs sm:text-sm font-medium text-gray-900">
                      ...
                      <div className="flex gap-2">
                        <Button variant="outline" onClick={handleCancelEdit} className="flex-1 text-xs sm:text-sm py-2.5 sm:py-2">
                          Anuluj
                        </Button>
                        <Button variant="primary" onClick={handleSaveEdit} className="flex-1 text-xs sm:text-sm py-2.5 sm:py-2">
                          Zapisz
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                        <div className="min-w-0 flex-1">
                          <h4 className="font-medium text-gray-900 text-sm sm:text-base truncate">{link.name}</h4>
                          <p className="text-xs sm:text-sm text-gray-600">
                            Utworzony: {formatDate(link.createdAt)}
                          </p>
                          {link.expiresAt && (
                            <p className={`text-xs sm:text-sm ${link.isExpired ? 'text-red-600' : 'text-gray-600'}`}>
                      ...
                      <div className="bg-gray-50 p-2 sm:p-3 rounded">
                        <p className="text-xs sm:text-sm text-gray-600 break-all">{link.url}</p>
                      </div>
                    </div>
                  )}
                </div>
