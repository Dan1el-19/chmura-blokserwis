...
export default function MobileActionsFab({
  ...
  return (
    <div
      className="md:hidden fixed bottom-4 right-4 z-[10000] flex flex-col items-end pb-safe"
      ref={panelRef}
      style={{ bottom: "max(1rem, env(safe-area-inset-bottom))" }}
    >
      {/* Expanded panel */}
      <div
        className={`origin-bottom-right transition-all duration-200 ${open ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-2 scale-95 pointer-events-none"} mb-3 w-44 sm:w-48 rounded-xl bg-white/95 fab-blur fab-shadow border border-gray-200 p-2.5 sm:p-3 space-y-2`}
      >
        {open && (
          <>
            <button
      ...
      >
        ...
        <span
          className={`block transition-transform duration-200 ${open ? "rotate-45" : "rotate-0"}`}
        >
          {open ? (
            <svg
              viewBox="0 0 24 24"
