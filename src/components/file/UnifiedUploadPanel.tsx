...
export default function UnifiedUploadPanel({ 
	...
	const multipartUploads = uploads.filter(upload => upload.engine === 'multipart');
	...
	return (
		<div className="mt-3 space-y-3">
			...
			{multipartUploads.map((upload) => {
				...
				const multipartData = upload.engineData as unknown as Record<string, unknown>;
				const multipartUpload: MultipartUploadState = {
					...
					status: (multipartData?.status as MultipartUploadStatus) || (upload.status as MultipartUploadStatus),
