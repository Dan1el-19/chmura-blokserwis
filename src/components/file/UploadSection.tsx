...
export default function UploadSection({
	...
	const handlePauseUpload = (fileId: string) => {
		setUploadFiles(prev =>
			prev.map(file =>
				file.id === fileId 
					? { ...file, status: file.status === "uploading" ? "paused" : "uploading" }
					: file
			)
		);
	};
			...
			<CardContent className="p-4 sm:p-6">
				...
				{uploadFiles.length > 0 && (
					<div className="space-y-3 mt-4">
						<h3 className="text-sm font-medium text-gray-700">Przesyłanie plików</h3>
									...
									<div className="flex items-center gap-1">
										...
										{uploadFile.status === 'paused' && (
											<>
												...
												<span className="text-xs text-gray-500">Wstrzymane</span>
											</>
										)}
										<button
