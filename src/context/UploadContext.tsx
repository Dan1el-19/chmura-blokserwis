  ...
  const value = useMemo<UploadContextValue>(() => {
        ...
        events: {
          onTaskUpdate: (task) => {
            ...
            if (task.status === "completed") {
              ...
              setTimeout(() => {
                setUploads((prev) =>
                  prev.filter((upload) => upload.id !== task.id)
                );
              }, 3000);
              // Odśwież listę plików
              onUploadComplete?.();
            } else if (task.status === "error") {
              void notificationManager.notifyUploadError(
                task.fileName,
                task.errorMessage || "Nieznany błąd"
              );
            }
          },
          onTaskComplete: (task) => {
            setUploads((prev) =>
              prev.map((u) => (u.id === task.id ? task : u))
            );
    ...
    return {
      ...
      enqueueMultipart: async (file, folder, subPath) => {
        try {
          const handle = await multipartManagerRef.current!.createUpload(
            file,
            folder,
            subPath
          );
          ...
          handle.onComplete = () => {
            ...
            setTimeout(() => {
              setMultipartUploads((prev) =>
                prev.filter((upload) => upload.id !== handle.id)
              );
            }, 3000);
          };
        } catch (error) {
      ...
      changePriority: (id, priority) =>
        managerRef.current!.changePriority(id, priority),
