...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const currentStorageUsed = userSnap.exists
        ? (userSnap.data() as UserDoc).storageUsed || 0
        : 0;
    ...
    const userProfile = {
      ...
      storageUsed,
      createdAt:
        userSnap.exists && userSnap.get("createdAt")
          ? ((userSnap.get("createdAt") as Timestamp).toDate?.() ?? new Date())
          : new Date(),
      lastLogin: new Date(),
    };
