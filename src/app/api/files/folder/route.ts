...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json().catch(() => null) as { folder?: string; name?: string } | null;
    if (!body || !body.name) {
      return NextResponse.json({ error: 'Brak nazwy folderu' }, { status: 400 });
    }
    const rawName = body.name.trim();
    if (!/^[\w\-. ]{1,64}$/.test(rawName)) {
      return NextResponse.json({ error: 'Nieprawidłowa nazwa folderu' }, { status: 400 });
    }
  ...
  const role = (decoded as { role?: string } | undefined)?.role || 'basic';
    ...
    const prefix = targetFolder === 'personal' ? `users/${decoded.uid}/` : 'main/';
    ...
    await client.send(new (await import('@aws-sdk/client-s3')).PutObjectCommand({
      Bucket: bucket,
      Key: folderKey,
      Body: '',
  ...
  return NextResponse.json({ success: true, key: folderKey, shortId, slug });
    ...
    return NextResponse.json({ error: 'Błąd tworzenia folderu' }, { status: 500 });
  }
}
