...
export async function GET(request: NextRequest) {
  ...
  const pathParam = (searchParams.get('path') || '').replace(/^\/+/, '').replace(/\.\./g,'');
      ...
      if (rest.endsWith('/.keep')) {
        ...
        if (!folderName.includes('/')) {
          ...
          if (!folderSet.has(firstSeg)) {
            ...
            if (db) {
              try {
                const metaSnap = await db.collection('folders').where('path','==', baseFolder.path).limit(1).get();
                if (!metaSnap.empty) {
                  ...
                  if (data.slug || data.shortId) baseFolder.slug = data.slug || `${baseFolder.name}-${data.shortId}`;
                }
              } catch {}
            }
  ...
  const folders = Array.from(folderSet.values()).sort((a,b)=> a.name.localeCompare(b.name,'pl'));
  return NextResponse.json({ folders, files, path: pathParam, base: basePrefix });
  } catch (e) {
