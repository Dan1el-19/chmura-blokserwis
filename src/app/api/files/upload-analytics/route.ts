...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalSize = uploadLogs.reduce(
    ...
    const queueStats = {
      ...
      completed: queueLogs.filter((log) => log.action === "queue_completed")
        .length,
      canceled: queueLogs.filter((log) => log.action === "queue_canceled")
        .length,
    };

    // Średni czas w kolejce (przybliżony)
    const queueTimes: number[] = [];
    const startedLogs = queueLogs.filter(
      (log) => log.action === "queue_started"
    );
    const addedLogs = queueLogs.filter((log) => log.action === "queue_added");

    startedLogs.forEach((startedLog) => {
      const addedLog = addedLogs.find(
        (added) => added.taskId === startedLog.taskId
      );
      if (addedLog && addedLog.timestamp && startedLog.timestamp) {
        const waitTime =
          startedLog.timestamp.toMillis() - addedLog.timestamp.toMillis();
