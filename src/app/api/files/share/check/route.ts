...
export async function GET(request: NextRequest) {
  try {
    ...
    if (slug.length > 50) {
      return NextResponse.json({ error: 'Slug za długi (max 50 znaków)', valid: false }, { status: 400 });
    }
    if (!/^[a-z0-9-]+$/.test(slug)) {
      return NextResponse.json({ error: 'Nieprawidłowy format slug', valid: false }, { status: 400 });
    }
