...
export async function GET(request: NextRequest) {
  try {
    ...
    // For uploads, extend expiration time to handle large files
    const expirationSeconds = op === 'put' ? 7200 : 300; // 2 hours for uploads, 5 minutes for downloads
    
    const responseParams = op === 'get' ? {
      'response-content-disposition': `attachment; filename="${encodeURIComponent(key.split('/').pop() || 'file')}"`
    } : undefined;

    const presignedUrl = await generatePresignedUrl(key, op, expirationSeconds, responseParams);
