...
export async function POST(request: NextRequest) {
  try {
    ...
    if (key.endsWith("/.keep")) {
      return NextResponse.json(
        { error: "Nie można zmienić nazwy folderu (MVP)" },
        { status: 400 }
      );
    }
    ...
    if (isMain) {
      ...
      if (role !== "admin" && role !== "plus") {
        return NextResponse.json(
          { error: "Brak uprawnień do plików głównych" },
          { status: 403 }
        );
      }
    }
    ...
    const { CopyObjectCommand, DeleteObjectCommand, HeadObjectCommand } =
    ...
    const httpStatus =
      ...
      e !== null &&
      "$metadata" in e &&
      typeof (e as Record<string, unknown>).$metadata === "object"
