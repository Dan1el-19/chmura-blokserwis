...
export async function GET(request: NextRequest) {
  try {
    ...
    snapshot.forEach(doc => {
      const data = doc.data();
      const expiresAt = data.expiresAt instanceof FieldValue ? null : data.expiresAt;
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Sprawdź uprawnienia do pliku
    if (!linkData.key.startsWith(`users/${decodedToken.uid}/`) && !linkData.key.startsWith('main/')) {
      return NextResponse.json({ error: 'Brak uprawnień do pliku' }, { status: 403 });
    }
  ...
  try {
    ...
    if (customExpiresAt || expiresIn) {
      ...
      if (customExpiresAt) {
        ...
        return NextResponse.json({ error: 'Nieprawidłowe dane wygaśnięcia' }, { status: 400 });
      }
