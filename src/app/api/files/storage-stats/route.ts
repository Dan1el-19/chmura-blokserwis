...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      personalStorageUsed = personalObjects.reduce(
    ...
    if (userRole === "admin" || userRole === "plus") {
      try {
        ...
        mainStorageUsed = mainObjects.reduce(
          (total, obj) => total + (obj.Size || 0),
          0
        );
        ...
        mainStorageLimit =
          userRole === "admin"
            ? 50 * 1024 * 1024 * 1024
            : 20 * 1024 * 1024 * 1024; // 50GB for admin, 20GB for plus
      } catch (error) {
    ...
    const stats = {
      ...
      main: {
        ...
        percentage:
          mainStorageLimit > 0 ? (mainStorageUsed / mainStorageLimit) * 100 : 0,
      },
      total: {
        used: personalStorageUsed + mainStorageUsed,
        limit: storageLimit + mainStorageLimit,
        percentage:
          ((personalStorageUsed + mainStorageUsed) /
            (storageLimit + mainStorageLimit)) *
          100,
      },
    };
