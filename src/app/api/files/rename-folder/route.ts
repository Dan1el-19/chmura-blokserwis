...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json().catch(()=>null) as { path?: string; newName?: string } | null;
    if (!body?.path || !body?.newName) return NextResponse.json({ error: 'Brak parametrów' }, { status: 400 });

    let { path, newName } = body;
    path = path.trim();
    newName = newName.trim();
    if (!path.endsWith('/')) return NextResponse.json({ error: 'Ścieżka musi kończyć się /' }, { status: 400 });
    if (!/^[\w\-. ]{1,64}$/.test(newName)) return NextResponse.json({ error: 'Nieprawidłowa nazwa' }, { status: 400 });
    ...
    if (!isPersonal && !isMain) return NextResponse.json({ error: 'Brak uprawnień' }, { status: 403 });
    if (isMain) {
      const role = (decoded as { role?: string } | undefined)?.role || 'basic';
      if (role !== 'admin' && role !== 'plus') return NextResponse.json({ error: 'Brak uprawnień do folderu głównego' }, { status: 403 });
    }

    // Determine new path (same parent)
    const parts = path.split('/').filter(Boolean); // remove empty
    if (parts.length === 0) return NextResponse.json({ error: 'Nieprawidłowa ścieżka' }, { status: 400 });
    ...
    for (const obj of objects) {
      ...
      await client.send(new CopyObjectCommand({ Bucket: bucket, Key: newKey, CopySource: `${bucket}/${obj.Key}` }));
    }
    ...
    if (db) {
      try {
        const snaps = await db.collection('folders').where('path','==', path).limit(1).get();
        if (!snaps.empty) {
          const doc = snaps.docs[0];
          const data = doc.data() as { shortId?: string; slug?: string };
          if (data?.shortId) newSlug = `${newName}-${data.shortId}`; // maintain same shortId
          await doc.ref.update({ name: newName, path: newPath, ...(newSlug ? { slug: newSlug } : {}), updatedAt: new Date() });
        }
      } catch (err) { console.error('Folder rename meta error', err); }
    }

    return NextResponse.json({ success: true, path: newPath, oldName, newName, slug: newSlug });
  } catch (e) {
    console.error('rename-folder error', e);
    return NextResponse.json({ error: 'Błąd zmiany nazwy folderu' }, { status: 500 });
  }
}
