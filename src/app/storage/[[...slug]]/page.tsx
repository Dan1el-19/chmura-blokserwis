...
function StoragePageInner() {
  ...
  // Auth Hook
  // =============================
  const { user, loading, userRole, getAuthToken, logout } = useStorageAuth();
  ...
  // =============================
  const { viewMode, sortField, sortDir, setViewMode, setSorting } =
    useViewPreferences();
  ...
  useEffect(() => {
    ...
    const folderDeleteHandler = async (e: Event) => {
      ...
      if (!confirm(`Usunąć folder "${folder.name}" wraz z zawartością?`))
        return;
      try {
    ...
    window.addEventListener(
      "cb:rename-folder-request",
      folderRenameHandler as EventListener
    );
    window.addEventListener(
      "cb:delete-folder-request",
      folderDeleteHandler as EventListener
    );
  ...
  const handleFolderRenamed = (info?: {
    newName: string;
    newPath: string;
    slug?: string;
  }) => {
    ...
    const oldSlug =
      fileOperations.renameFolderTarget.slug ||
      fileOperations.renameFolderTarget.name;
