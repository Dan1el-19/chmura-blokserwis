<script lang="ts">
	...
	let list = $derived(data.quotations.items);
...
<div class="mx-auto max-w-7xl space-y-5">
	...
	>
		...
		><label
			><span class="sr-only">Status</span><select
				...
				><option value="archived">Archiwalne</option></select
			></label
		><label
	...
	{#if list.length === 0}<section
		...
		>
			...
			<h2 class="mt-4 text-base font-semibold text-text-main">Brak wycen dla tych filtrów</h2>
			...
			class="overflow-x-auto rounded-md border border-border-line bg-bg-panel"
		>
			<table class="w-full min-w-230 text-left text-sm">
				<thead class="bg-bg-app text-xs text-text-muted"
					><tr
						><th class="px-4 py-3 font-medium">Numer / tytuł</th><th class="px-4 py-3 font-medium"
							>Klient</th
						><th class="px-4 py-3 text-right font-medium">Wartość brutto</th><th
							class="px-4 py-3 font-medium">Status</th
						><th class="px-4 py-3 font-medium">Autor</th><th class="px-4 py-3 font-medium"
							>Aktualizacja</th
						><th class="px-3 py-3"><span class="sr-only">Akcje</span></th></tr
					></thead
				><tbody
					>{#each list as quotation (quotation.id)}<tr class="border-t border-border-line"
							><td class="px-4 py-3"
								><a
							...
							><td class="px-4 py-3 text-right font-mono text-text-main"
								>{money.format(quotation.document.totalGrossCents / 100)}</td
							><td class="px-4 py-3"
								><span
							...
							><td class="px-3 py-2"
								><div class="flex justify-end gap-1">
									...
									>{#if quotation.status === 'archived'}<button
											type="button"
											disabled={busyId === quotation.id}
