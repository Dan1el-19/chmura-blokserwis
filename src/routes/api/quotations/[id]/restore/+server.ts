...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		return json(await client.quotations.restore(event.params.id, withoutProxyFields(body)));
	} catch (error) {
		return quotationErrorResponse(error, 'Nie udało się przywrócić wyceny.');
	}
};
