import { UnifiedUploadState, UppyUploadData, SimpleUploadData } from '@/types/upload';
...
export function convertSimpleUploadToUnified(
	...
	return {
		...
		status: simpleUpload.status === 'completed' ? 'success' : simpleUpload.status,
		progress: simpleUpload.smoothProgress || (simpleUpload.uploadedBytes / simpleUpload.size) * 100,
...
export function convertUppyJobToUnified(
	uppyJob: {
		progress: number;
...
export function convertMultipartUploadToUnified(
	...
	return {
		...
		status: multipartUpload.status === 'completed' ? 'success' : multipartUpload.status,
