...
export class UploadQueueManager {
  ...
  private taskMetadata: Map<string, { userId?: string; fileName?: string }> =
    new Map();
  ...
  public getQueueStats(): QueueStats {
    ...
    const averageWaitTime =
      this.queue.length > 0
        ? this.queue.reduce(
            (sum, item) => sum + (now - item.addedAt) / 1000,
            0
          ) / this.queue.length
        : 0;
    ...
    return {
      ...
      averageWaitTime,
      estimatedThroughput:
        totalTime > 0 ? totalSize / (1024 * 1024) / totalTime : 0,
    };
  }
  ...
  public changePriority(taskId: string, newPriority: number): boolean {
    ...
    const insertIndex = this.queue.findIndex(
      (qItem) => qItem.priority > newPriority
    );
    if (insertIndex === -1) {
  ...
  public cancelAll(): string[] {
    const allIds = Array.from(this.activeUploads).concat(
