...
export async function generatePresignedUrl(
  ...
  let actualExpiration = isLikelyLargeFile ? Math.max(expiresIn, 7200) : expiresIn; // min 2 hours for large files

  // For GET (preview) enforce at least PREVIEW_URL_MIN_SECONDS (default 300s)
  if (op === 'get') {
    const previewMin = parseInt(process.env.PREVIEW_URL_MIN_SECONDS || '300', 10);
    if (!isNaN(previewMin)) {
  ...
  if (op === 'put') {
    ...
    const commandParams = { 
      Bucket: bucket, 
      Key: key,
      ResponseContentDisposition: responseParams?.['response-content-disposition']
    };
...
export async function headObjectMeta(key: string): Promise<{ size: number | null; contentType: string | null; }>{
  const client = initializeS3Client();
  const bucket = getBucketName();
  const res = await client.send(new HeadObjectCommand({ Bucket: bucket, Key: key }));
  return {
