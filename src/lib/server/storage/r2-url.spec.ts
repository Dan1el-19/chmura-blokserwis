...
describe('R2 URL validation', () => {
	it('accepts a virtual-hosted presigned URL for the configured bucket', () => {
		expect(() =>
			assertPresignedUrlMatchesR2Config(
				`https://${bucket}.00000000000000000000000000000000.r2.cloudflarestorage.com/uploads/file.jpg?X-Amz-Signature=abc`,
				endpoint,
				bucket
			)
		).not.toThrow();
	...
	it('rejects a presigned URL generated against a different bucket host', () => {
		expect(() =>
			assertPresignedUrlMatchesR2Config(
				`https://blokserwis.00000000000000000000000000000000.r2.cloudflarestorage.com/${bucket}/uploads/file.jpg?X-Amz-Signature=abc`,
				endpoint,
				bucket
			)
	...
	it('rejects a Cloudflare R2 endpoint that includes an extra bucket subdomain', () => {
		expect(() =>
			assertR2Endpoint(
				'https://blokserwis.00000000000000000000000000000000.r2.cloudflarestorage.com'
			)
