...
export class MultipartUploadManager {
  ...
  async startUpload(
    ...
    if (folder === "main") {
      ...
      key = subPath
        ? `users/${userId}/${subPath}/${file.name}`
  ...
  private async uploadPart(
    ...
    const chunkInfo = this.chunkInfo
      .get(uploadId)
    ...
    try {
      ...
      const presignedUrl = await this.getPresignedUrlForPart(
        upload,
        partNumber
      );
      ...
      const existingPartIndex = upload.parts.findIndex(
      ...
      if (
        this.areAllChunksCompleted(uploadId) &&
        !this.completionInProgress.has(uploadId)
      ) {
  ...
  async completeUpload(uploadId: string): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "UPLOAD_EXPIRED") {
        console.log(
          "⚠️ Upload expired during finalization, marking as completed anyway:",
          uploadId
        );
        upload.status = "completed";
        upload.progress = 100;
        ...
        try {
          await this.recordUploadInSystem(upload);
          console.log(
            "📝 Upload recorded in system despite R2 expiration:",
            uploadId
          );
        } catch (recordError) {
  ...
  private async syncCompletedPartsFromServer(uploadId: string): Promise<void> {
    ...
    const response = await fetch(
      `/api/files/multipart/list-parts?uploadId=${upload.uploadId}&key=${upload.key}`,
      {
        method: "GET",
        headers: {
    ...
    if (chunks) {
      for (const chunk of chunks) {
        const serverPart = parts.find(
          (p: Part) => p.partNumber === chunk.partNumber
        );
        if (serverPart) {
          ...
          console.log(
            `✅ Chunk ${chunk.partNumber} confirmed completed on server`
          );
          ...
          console.log(
            `🔄 Chunk ${chunk.partNumber} needs re-upload (not on server)`
          );
        }
      }
    }
    ...
    upload.bytesUploaded = completedChunks.reduce(
      (sum, chunk) => sum + chunk.size,
      0
    );
    upload.progress = Math.round(
      (upload.bytesUploaded / upload.fileSize) * 100
    );
  ...
  getUploadStats(uploadId: string): MultipartUploadStats | null {
    ...
    const chunks = this.chunkInfo.get(uploadId) || [];
    const completedParts = chunks.filter(
  ...
  private calculateOptimalChunks(file: File): ChunkInfo[] {
    ...
    if (!shouldUseMultipart(fileSize)) {
      ...
      throw new Error(
        "File too small for multipart upload. Use simple upload instead."
      );
    }
    ...
    this.options.maxConcurrentChunks =
      optimization.metadata.concurrencyRecommendation;
   ...
   */
  private async initiateUploadOnR2(
   ...
   */
  private async getPresignedUrlForPart(
    upload: MultipartUploadState,
    partNumber: number
  ): Promise<string> {
    const token = await auth.currentUser?.getIdToken();
  ...
  private async uploadChunkToR2(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        reject(
          new Error(`Upload ${uploadId} is ${upload?.status || "not found"}`)
        );
        return;
      }
          ...
          if (currentUpload && handle) {
            ...
            if (chunks) {
              ...
              if (chunkInfo) {
                ...
                const totalUploadedBytes = chunks.reduce(
                  (sum, c) => sum + (c.uploadedBytes || 0),
                  0
                );
                const totalProgress = Math.round(
                  (totalUploadedBytes / currentUpload.fileSize) * 100
                );
      ...
      xhr.addEventListener("load", () => {
        ...
        if (xhr.status !== 200) {
          ...
          reject(
            new Error(`Failed to upload chunk: ${xhr.status} ${xhr.statusText}`)
          );
          return;
        }
   ...
   */
  private async finalizeUploadOnR2(
    upload: MultipartUploadState
  ): Promise<void> {
    const token = await auth.currentUser?.getIdToken();
    if (!token) throw new Error("No auth token");
    ...
    console.log(
      "🔧 Finalizing upload with sorted parts:",
    ...
    if (!response.ok) {
      ...
      if (response.status === 410 && error.code === "UPLOAD_NOT_FOUND") {
        console.warn(
          "⚠️ Upload session expired, cleaning up state:",
          upload.uploadId
        );
   ...
   */
  private async recordUploadInSystem(
    upload: MultipartUploadState
  ): Promise<void> {
    const token = await auth.currentUser?.getIdToken();
    if (!token) throw new Error("No auth token");
  ...
  private tryStartNextChunk(uploadId: string): void {
    ...
    if (activeCount >= maxConcurrent) {
      console.log(
        `⏭️ Max concurrent chunks reached (${activeCount}/${maxConcurrent}), waiting...`
      );
      return;
    }

    // Znajdź pierwszy pending chunk
    const nextPendingChunk = chunks.find(
  ...
  async uploadChunk(uploadId: string, partNumber: number): Promise<void> {
    ...
    if (upload.status === "paused") {
      console.log(
        `⏸️ Upload ${uploadId} is paused, stopping chunk ${partNumber}`
      );
      return;
    }
    ...
    if (upload.status === "aborted") {
      console.log(
        `🛑 Upload ${uploadId} is aborted, stopping chunk ${partNumber}`
      );
      return;
    }
    ...
    if (
      this.activeChunks.get(uploadId)?.has(partNumber) &&
      chunkInfo.status === "uploading"
    ) {
    ...
    try {
      ...
      const chunk = await this.getFileChunk(
        uploadId,
        chunkInfo.startByte,
        chunkInfo.endByte
      );
      if (chunk) {
        ...
        console.log(
          "✅ Chunk completed via retry path, finalization handled elsewhere"
        );
      } else {
      ...
      if (error instanceof Error && error.message === "UPLOAD_EXPIRED") {
        console.log(
          `🔄 Upload expired for chunk ${partNumber}, will need to restart entire upload`
        );
        upload.status = "error";
        upload.errorMessage =
  ...
  private async retryChunkUpload(
    ...
    console.log(
      `Retrying chunk ${partNumber} in ${delay}ms (attempt ${newRetryCount}/${this.options.retryAttempts})`
    );
    ...
    try {
      await this.uploadPart(uploadId, partNumber, chunk);
      console.log(
        `✅ Retry ${newRetryCount} succeeded for chunk ${partNumber}`
      );
    } catch (error) {
      // Sprawdź czy to błąd z powodu pause/abort
      if (
        ...
        console.log(
          `⏸️ Retry ${newRetryCount} stopped due to pause/abort for chunk ${partNumber}`
        );
        throw error; // Rzuć błąd dalej ale bez dodatkowego logu
      }
  ...
  private calculateProgress(upload: MultipartUploadState): number {
    ...
    const completedBytes = completedChunks.reduce(
      (sum, chunk) => sum + chunk.size,
      0
    );
  ...
  private updateSpeedStats(upload: MultipartUploadState): void {
    ...
    if (elapsedSec > 0) {
      ...
      if (upload.speed > 0) {
        upload.etaSec = Math.round(
          remainingBytes / (upload.speed * 1024 * 1024)
        );
      }
    }
  }
