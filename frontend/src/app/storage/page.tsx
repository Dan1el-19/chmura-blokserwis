...
function StoragePageInner() {
  ...
  // Ensure we destroy any active Uppy handle when it changes or component unmounts
  useEffect(() => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      // Duże pliki → kalkulator, potem Uppy
      if (useUppy && file.size >= LARGE_FILE_THRESHOLD) {
        setSelectedFileForUpload(file);
        setShowCostCalculatorModal(true);
        break; // tylko pierwszy duży plik na raz
      }
      // Małe pliki → backend
  ...
  const handleCostCalculatorConfirm = async () => {
    ...
    const fileToProcess = selectedFileForUpload;
    const isLarge = useUppy && fileToProcess.size >= LARGE_FILE_THRESHOLD;
    if (isLarge) {
      try {
        const handle = await startUppyUploadWithProgress(fileToProcess, currentFolder as 'personal' | 'main', {
          onStarted: () => setUppyJob(prev => prev ? { ...prev, status: 'uploading' } : { status: 'uploading', progress: 0, handle: null, fileName: fileToProcess.name, fileSize: fileToProcess.size }),
          onProgress: (percent, bytesUploaded, bytesTotal, speedBps) => setUppyJob(prev => ({
            ...(prev || { status: 'uploading', progress: 0, handle: null }),
            progress: percent,
            speedBps,
            bytesUploaded,
            fileSize: bytesTotal
          ...
          onSuccess: () => {
            setUppyJob(prev => prev ? { ...prev, status: 'success', progress: 100 } : { status: 'success', progress: 100, handle: null });
          ...
          onError: (msg) => {
            toast.error(msg);
            setUppyJob(prev => prev ? { ...prev, status: 'error' } : { status: 'error', progress: 0, handle: null });
          },
          onCancel: () => {
            setUppyJob(prev => prev ? { ...prev, status: 'error' } : { status: 'error', progress: 0, handle: null });
          ...
          onPaused: () => {
            setUppyJob(prev => prev ? { ...prev, status: 'paused' } : prev);
          },
          onResumed: () => {
            setUppyJob(prev => prev ? { ...prev, status: 'uploading' } : prev);
          },
          onRestored: (percent) => {
            setUppyJob({ progress: percent, status: 'restored', handle: null });
          }
        });
        setUppyJob({ progress: 0, status: 'uploading', handle, fileName: fileToProcess.name, fileSize: fileToProcess.size });
      } catch (e) {
      ...
      setShowCostCalculatorModal(false);
      setSelectedFileForUpload(null);
      await enqueue(fileToProcess, currentFolder as 'personal' | 'main');
    }
  };
            ...
            {uppyJob && (
              <div className="mt-3 p-3 bg-white border border-gray-200 rounded-md">
                <div className="flex items-center justify-between mb-2 text-sm text-gray-700">
                  ...
                  <span>{uppyJob.status === 'uploading' ? 'Wysyłanie…' : uppyJob.status === 'success' ? 'Zakończone' : 'Błąd'}</span>
                </div>
                <div className="w-full">
                  <div className="flex items-center justify-between mb-1 text-xs text-gray-600">
                    <span>
                      {uppyJob.fileName ? `${uppyJob.fileName}` : ''}
                      {typeof uppyJob.bytesUploaded === 'number' && typeof uppyJob.fileSize === 'number' && (
                        ` • ${formatFileSize(uppyJob.bytesUploaded)} / ${formatFileSize(uppyJob.fileSize)}`
                      )}
                      {uppyJob.speedBps ? ` • ${(uppyJob.speedBps / (1024 * 1024)).toFixed(2)} MB/s` : ''}
                    </span>
                    <span>
                      {uppyJob.status === 'paused' && <span className="italic">Wstrzymane</span>}
                      {uppyJob.status === 'uploading' && uppyJob.speedBps && uppyJob.fileSize && typeof uppyJob.bytesUploaded === 'number' && (() => {
                        const remaining = Math.max(0, uppyJob.fileSize - uppyJob.bytesUploaded);
                        const etaSec = remaining / (uppyJob.speedBps || 1);
                        return <span className="text-gray-500">ETA: {formatDuration(etaSec)}</span>;
                      })()}
                    </span>
                  </div>
                  <div className="h-2 bg-gray-200 rounded">
                    <div className={`h-2 rounded ${uppyJob.status === 'error' ? 'bg-red-500' : 'bg-blue-600'}`} style={{ width: `${uppyJob.progress}%` }} />
                  </div>
                </div>
                {uppyJob.handle && (
                  <div className="mt-2 flex items-center justify-end gap-2">
                    {uppyJob.status === 'uploading' && (
                      <>
                        <button
                          onClick={() => { try { uppyJob.handle?.pause(); } catch {} }}
                          className="px-2 py-1 bg-yellow-600 text-white rounded text-xs"
                        >Pauza</button>
                        <button
                          onClick={() => {
                            try { uppyJob.handle?.cancel?.(); } catch {}
                            try { uppyJob.handle?.destroy?.(); } catch {}
                            setUppyJob(null);
                          }}
                    ...
                    {(uppyJob.status === 'paused' || uppyJob.status === 'restored') && (
                      <>
                        <button
                          onClick={() => { try { uppyJob.handle?.resume(); } catch {} }}
