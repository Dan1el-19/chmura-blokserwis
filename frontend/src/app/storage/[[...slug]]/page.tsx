...
import { formatFileSize } from '@/lib/costCalculator';
...
function StoragePageInner() {
	...
	// Upload (Uppy) State
	// =============================
	const useUppy = (process.env.NEXT_PUBLIC_USE_UPPY ?? 'true') !== 'false';
	...
	// Destroy uppy handle on unmount/change
	useEffect(() => {
		const handle = uppyJob?.handle;
		return () => {
			try {
				handle?.destroy();
			} catch {}
	...
	const handleFileUpload = async (
		...
		for (let i = 0; i < filesSel.length; i++) {
			const f = filesSel[i];
			if (useUppy && f.size >= LARGE_FILE_THRESHOLD) {
				setSelectedFileForUpload(f);
				setShowCostCalculatorModal(true);
				break; // open cost calculator - remaining files queued after confirm
			}
				...
				const handle = await startUppyUploadWithProgress(
					...
					subPath || undefined,
					{
						...
						onSuccess: () => {
							...
							setTimeout(() => setUppyJob(null), 1500);
							fetchFiles();
							fetchUserData();
						},
						onError: (msg: string) => {
	...
	const handleFileDownload = async (file: FileItem) => {
		try {
			const response = await fetch(
				`/api/files/download?key=${encodeURIComponent(file.key)}`,
				{ headers: { Authorization: `Bearer ${await getAuthToken()}` } }
			);
			if (response.ok) {
				...
				toast.error(
					`Błąd podczas pobierania pliku: ${errorData.error || 'Nieznany błąd'}`
				);
			}
		} catch {
		...
		<div className="min-h-screen bg-gray-50">
			...
			<main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
				<div className="space-y-4">
					{path.length > 0 && (
						<nav
							className="text-sm text-gray-600 flex items-center flex-wrap gap-1"
							aria-label="Ścieżka"
						>
							<button
					...
					<StorageUsageBars refreshToken={storageRefreshTick} />
						...
						{/* Uppy Upload Progress Panel */}
						{uppyJob && (
							<div className="mt-3 p-3 bg-white border border-gray-200 rounded-md">
								<div className="flex items-center justify-between mb-2 text-sm text-gray-700">
									<span>Upload (Uppy)</span>
									<span>
										{uppyJob.status === 'uploading'
											? 'Wysyłanie…'
											: uppyJob.status === 'success'
											? 'Zakończone'
											: 'Błąd'}
									</span>
								</div>
								<div className="w-full">
									<div className="flex items-center justify-between mb-1 text-xs text-gray-600">
										<span>
											{uppyJob.fileName ? uppyJob.fileName : ''}
											{typeof uppyJob.bytesUploaded === 'number' &&
												typeof uppyJob.fileSize === 'number' &&
												` • ${formatFileSize(uppyJob.bytesUploaded)} / ${formatFileSize(
													uppyJob.fileSize
												)}`}
											{uppyJob.speedBps
												? ` • ${(uppyJob.speedBps / (1024 * 1024)).toFixed(
														2
													)} MB/s`
												: ''}
										</span>
										<span>
											{uppyJob.status === 'paused' && (
												<span className="italic">Wstrzymane</span>
											)}
											{uppyJob.status === 'uploading' &&
												uppyJob.speedBps &&
												uppyJob.fileSize &&
												typeof uppyJob.bytesUploaded === 'number' && (() => {
													const remaining = Math.max(
														0,
														uppyJob.fileSize - (uppyJob.bytesUploaded || 0)
													);
													const etaSec = remaining / (uppyJob.speedBps || 1);
													return (
									...
									<div className="h-2 bg-gray-200 rounded">
										<div
											className={`h-2 rounded ${
												uppyJob.status === 'error' ? 'bg-red-500' : 'bg-blue-600'
											}`}
											style={{ width: `${uppyJob.progress}%` }}
										/>
									</div>
								</div>
								{uppyJob.handle && (
									<div className="mt-2 flex items-center justify-end gap-2">
										{uppyJob.status === 'uploading' && (
											<>
												...
												<button
													onClick={() => {
														try {
															uppyJob.handle?.cancel?.();
														} catch {}
														try {
															uppyJob.handle?.destroy?.();
														} catch {}
														setUppyJob(null);
													}}
										...
										{(uppyJob.status === 'paused' ||
											uppyJob.status === 'restored') && (
											<>
												<button
													onClick={() => {
														try {
															uppyJob.handle?.resume();
														} catch {}
													}}
										...
										{uppyJob.status === 'error' && (
											<button
												onClick={() => setUppyJob(null)}
						...
						{/* Main File Section */}
						<div className="mt-6 rounded-lg transition-all duration-200 glass-card bg-white/80 backdrop-blur-sm border border-gray-200/50 shadow-xl">
							<div className="px-4 sm:px-6 py-4 border-b border-gray-200 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
								{/* Left Controls */}
								<div className="flex items-center gap-3 flex-wrap">
									<h3 className="text-lg font-medium text-gray-900">Pliki</h3>
									<span className="text-xs text-gray-500 hidden sm:inline">
										{files.length} pozycji
									</span>
									<select
										aria-label="Sortowanie"
										className="h-9 rounded-md border border-gray-300 bg-white px-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
									...
									>
										...
										<option value="size:asc">Sortuj: Rozmiar ↑</option>
										<option value="size:desc">Sortuj: Rozmiar ↓</option>
										<option value="lastModified:desc">Sortuj: Najnowsze</option>
										<option value="lastModified:asc">Sortuj: Najstarsze</option>
									</select>
									<div className="flex items-center gap-1">
										<button
											onClick={() => setViewMode('grid')}
											aria-label="Widok siatki"
											className={`h-9 w-9 inline-flex items-center justify-center rounded-md border transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 ${
												viewMode === 'grid'
									...
									<button
										...
										className={`h-9 px-3 inline-flex items-center gap-2 rounded-md border text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 ${
											multiSelectMode
												? 'border-blue-600 text-blue-600 bg-blue-50'
												: 'border-gray-300 text-gray-600 hover:bg-gray-50'
										}`}
								...
								<div className="flex items-center gap-2 flex-wrap">
									<button
										...
										className={`h-9 px-4 inline-flex items-center gap-2 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 shadow-sm ${
											hasActiveUploads
									...
									<button
										...
										className="h-9 px-4 inline-flex items-center gap-2 rounded-md border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
									>
										+ Folder
									</button>
								</div>
