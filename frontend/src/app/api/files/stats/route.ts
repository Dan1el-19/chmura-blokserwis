...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!fileKey.startsWith(`users/${decodedToken.uid}/`) && !fileKey.startsWith('main/')) {
    ...
    const linksSnapshot = await db.collection('sharedFiles')
      .where('key', '==', fileKey)
      .get();
    ...
    try {
      ...
      if (linkIds.length <= 10) {
        const usageSnapshot = await db.collection('linkUsage')
        ...
        usageData = usageSnapshot.docs.map(doc => {
          ...
          return {
            ...
            accessedAt: data.accessedAt instanceof Timestamp ? data.accessedAt.toDate() : data.accessedAt,
    ...
    const linkStats = linkIds.map(linkId => {
      ...
      return {
        ...
        createdAt: linkData?.createdAt instanceof Timestamp ? linkData.createdAt.toDate() : linkData?.createdAt
      };
    });
