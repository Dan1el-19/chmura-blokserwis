...
if (!email) {
  console.error('Usage: node scripts/set-admin.mjs <UID|AUTO> <EMAIL> [DISPLAY_NAME] [LIMIT_GB]');
  process.exit(1);
}
...
async function resolveUidAndEnsureUser() {
  ...
  const created = await auth.createUser({ email, displayName, password: tempPassword, emailVerified: false, disabled: false });
  console.log('Created user:', created.uid);
  console.log('Temp password (share securely, ask user to change):', tempPassword);
  return created.uid;
}
